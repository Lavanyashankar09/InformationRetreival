...
def compute_term_frequency_sum(doc_vectors):
    term_frequency_sum = {}
    for vec in doc_vectors:
        for term, freq in vec.items():
            term_frequency_sum[term] = term_frequency_sum.get(term, 0) + freq
    return term_frequency_sum


def compute_log_likelihood(term_freq_label1, term_freq_label2, epsilon=0.2):
    log_likelihood = {}
    for term in term_freq_label2:
        if term in term_freq_label1 and term_freq_label1[term] > 0 and term_freq_label2[term] > 0:
            log_likelihood[term] = np.log(term_freq_label1[term] / term_freq_label2[term])
        else:
            log_likelihood[term] = np.log(epsilon / (term_freq_label2[term] + epsilon))  # Avoid division by zero

    for term in term_freq_label1:
        if term not in term_freq_label2:
            log_likelihood[term] = np.log(term_freq_label1[term] / epsilon)
...
def classify_document(doc, vec, log_likelihood):
    sumofLL = sum(log_likelihood.get(term, 0) * freq for term, freq in vec.items())
    ...
    return predicted_class, sumofLL

def evaluate_classification(doc, predicted_class):
    return (doc.output == predicted_class) if predicted_class != 0 else False

def classify_documents(dev_docs, dev_vectors, log_likelihood,dataset):
    terminal_results = {}
    file_results = ""
    total_correct = 0
    total_incorrect = 0

    for i, (doc, vec) in enumerate(zip(dev_docs, dev_vectors)):
        predicted_class, sumofLL = classify_document(doc, vec, log_likelihood)
        is_correct = evaluate_classification(doc, predicted_class)
        ...
        terminal_results[doc.doc_id] = predicted_class  # Assuming doc_id is the unique identifier
        file_results += f"Vec #{i + 1},\tTrue Class: {doc.output},\tPredicted Class: {predicted_class},\tSum of LL: {sumofLL}\n"
    ...
    #print(f"Terminal Results: {terminal_results}")
    print(f"Accuracy for dataset {dataset}: {accuracy}")

    return accuracy, terminal_results
...
def all(stem, removestop, term_weights, tokenization_mode, sim):
    ...
    for term, dataset in it.product(*permutations):
        ...
        term_freq_sum_label1 = compute_term_frequency_sum(label_1_train_vectors)
        term_freq_sum_label2 = compute_term_frequency_sum(label_2_train_vectors)

        log_likelihood = compute_log_likelihood(term_freq_sum_label1, term_freq_sum_label2)

        accuracy, results = classify_documents(dev_docs, dev_vectors, log_likelihood,dataset)
