...
class SegmentClassifier:
    ...
    def extract_features(self, text):
        ...
        features = [  # TODO: add features here
            ...
            ratio(words),
            totalNum(words),
            sum(1 if colon(w) else 0 for w in words),
        ]
        return features
...
def totalNum(words):
    return sum(1 for w in words if any(char.isdigit() for char in w))

def ratio(words):
    num1, words1 = 0, 0
    for w in words:
        num1 += 1 if any(char.isdigit() for char in w) else 0
        words1 += 1 if not any(char.isdigit() for char in w) else 0
    return (num1 - 1) if words1 == 0 else (num1 / words1)
