...
def experiment():
    ...
    processed_docs = Top_docs(docs, stem, removestop, stopwords)
    doc_freqs = compute_doc_freqs(processed_docs)
    doc_vectors = [term_func(doc, doc_freqs, term_weights) for doc in processed_docs]

    # User Input
    print('\nEnter "quit" to exit user input\n')

    while True:
        # User input for document query
        author_string = input('Enter Author: ')
        if author_string.lower() == 'quit':
            break
        title_string = input('Enter Title: ')
        keyword_string = input('Enter Keyword: ')
        abstract_string = input('Enter Abstract: ')
        ...
        print('\nTop 10 document IDs matching your query:')
        for doc_id in results[:10]:
            print(f'Document ID: {doc_id}')

        # Ask user if they want to see more results
        choice = input('\nDo you want to see more documents? (y/n): ').lower()
        if choice != 'y':
            break
...
def Top_docs(docs, stem, removestop, stopwords):
    processed_docs = docs
    if removestop:
