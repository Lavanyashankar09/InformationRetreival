...
def bigrams(docs):
  ...
  def gen(words):
    ...
    for i in range(len(words) - 1):
        word1 = words[i]
        word2 = words[i + 1]
        if word1 not in stopwords and word1 not in punctuation:
  ...
  for doc in docs:
      bigramAuthor = gen(doc.author)
      bigramTitle = gen(doc.title)
      bigramKeyword = gen(doc.keyword)
      bigramAbstract = gen(doc.abstract)
      bigram_doc = Document(doc.doc_id, doc.author + bigramAuthor,doc.title + bigramTitle,doc.keyword + bigramKeyword,doc.abstract + bigramAbstract) 
      bigram_docs.append(bigram_doc)  
  return bigram_docs
